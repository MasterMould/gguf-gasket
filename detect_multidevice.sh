...
# Primary GPU detection - FIXED for Intel Arc
detect_gpu() {
    ...
    # Check if any line has both "Intel" AND a GPU indicator
    while IFS= read -r line; do
        if echo "$line" | grep -iq "Intel"; then
            if echo "$line" | grep -iqE "VGA|3D|Display|Graphics|Arc|DG"; then
                # Found an Intel GPU line - now check if AMD is present
...
select_gpu_with_override() {
    ...
    if [ -d "$PROFILE_DIR" ]; then
        ...
        for profile in "$PROFILE_DIR"/*.profile; do
            ...
            if [ "$HOSTNAME" = "$current_hostname" ]; then
                ...
                INFO "Device profile active: $pname (GPU: $GPU_TYPE)"
                has_profile=1
                break
            fi
        done
    ...
    if [[ -f "$override_file" ]] && [[ $has_profile -eq 0 ]]; then
        ...
        INFO "Manual override active: $current_override"
    fi
