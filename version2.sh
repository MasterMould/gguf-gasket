...
draw_header() {
    echo -e "${CLEAR}${B_CYAN}======================================================"
    echo -e "           🦙 LLAMA COMMAND CENTER (v7.1) 🦙"
...
build_engine() {
    ...
    # Base Dependencies
    echo "Installing base dependencies..." | tee -a "$LOG_FILE"
    sudo apt-get update && sudo apt-get install -y build-essential git curl cmake libcurl4-openssl-dev libssl-dev
    ...
    cd "$INSTALL_DIR" || { echo -e "${B_RED}Failed to enter install directory.${NC}"; exit 1; }
    ...
    echo "Running cmake with flags: $CMAKE_FLAGS" | tee -a "$LOG_FILE"
    ...
    if cmake --build . --config Release -j$(nproc); then
        ...
        echo -e "\n${B_RED}✖ Build failed. Check $LOG_FILE for details.${NC}"
    fi
    read -p "Press Enter to return..."
}

# --- 📥 MODEL BROWSER (Improved) ---
download_model() {
    draw_header
    ...
    if curl -L -# "$U" -o "$MODEL_DIR/$F"; then
        echo -e "${B_GREEN}Download finished!${NC}"
    else
        echo -e "${B_RED}Download failed. Check your connection.${NC}"
    fi
    read -p "Press Enter..."
}
...
manage_server() {
    ...
    echo -e "${B_YELLOW}⚠️ Self-signed certs are not trusted by browsers by default.${NC}"
    ...
    echo -e "${B_YELLOW}You may need to accept the security risk in your browser.${NC}"
    read -p "Press Enter to return..."
}
