...
# ── Section 10: SYCL Memory & Runtime Crash Diagnostics ──────────────────────
check_sycl_memory() {
    section "10 · SYCL Memory & Runtime Diagnostics"

    # ── USM allocator env vars ──
    echo -e "  ${D}USM memory pool (prevents fragmentation OOM crashes)${N}"
    echo ""

    local -A sycl_mem_vars=(
        ["SYCL_PI_LEVEL_ZERO_USM_ALLOCATOR"]="1"
    ...
    for var in "${!sycl_mem_vars[@]}"; do
        local cur="${!var:-}"
        local want="${sycl_mem_vars[$var]}"
        if [[ -n "$cur" ]]; then
            print_row "$TICK" "$var" "${G}=${cur}${N}"
            log_result PASS "$var set"
        else
            print_row "$WARN" "$var" "${Y}not set — recommended: ${want}${N}"
            log_result WARN "$var not set"
            needs_profile_update=true
        fi
    done

    # ── Fix: write all missing vars to the profile ──
    if $needs_profile_update; then
        echo ""
        echo -e "  ${FIX} Writing missing SYCL memory vars to /etc/profile.d/arc-llamacpp.sh…"
        if ensure_sudo; then
            sudo bash -c 'cat >> /etc/profile.d/arc-llamacpp.sh << '"'"'SYCL_MEM_VARS'"'"'

# SYCL memory pool & runtime settings — added by arc-fix.sh (SYCL OOM fix)
# Prevents "could not create a memory object" crashes on Arc A770
export SYCL_PI_LEVEL_ZERO_USM_ALLOCATOR=1
export ZE_FLAT_DEVICE_HIERARCHY=COMPOSITE
SYCL_MEM_VARS'
            sudo chmod 644 /etc/profile.d/arc-llamacpp.sh
            record_fix "Added SYCL_PI_LEVEL_ZERO_USM_ALLOCATOR + ZE_FLAT_DEVICE_HIERARCHY to profile"
            log_result PASS "SYCL memory pool vars written to profile"
        fi
    fi

    # ── Check GDB auto-attach crash handler noise ──
    echo ""
    echo -e "  ${D}GDB auto-attach suppression${N}"
    local gdb_conf="${HOME}/.config/gdb/gdbinit"
    local sys_gdb="/etc/gdb/gdbinit"

    local gdb_ok=false
    if grep -q "set auto-load safe-path /" "$gdb_conf" 2>/dev/null || \
       grep -q "set auto-load safe-path /" "$sys_gdb" 2>/dev/null; then
        print_row "$PASS" "GDB auto-load path" "safe-path set"
        log_result PASS "GDB auto-load safe-path configured"
        gdb_ok=true
    fi

    if ! $gdb_ok; then
        print_row "$WARN" "GDB auto-load" "${Y}not configured — SYCL crash dumps show spurious warnings${N}"
        log_result WARN "GDB auto-load safe-path not set"
        echo ""
        echo -e "  ${FIX} Adding GDB config to suppress SYCL debuginfo warnings…"
        if ensure_sudo; then
            mkdir -p "$(dirname "$gdb_conf")"
            cat >> "$gdb_conf" << 'GDBEOF'
# Suppress SYCL/oneAPI auto-load warnings
set auto-load safe-path /
set debuginfod enabled off
GDBEOF
            record_fix "Wrote ~/.config/gdb/gdbinit (GDB SYCL warning suppression)"
            log_result PASS "GDB auto-load configured"
        fi
    fi

    # ── VRAM headroom calculator ──
    echo ""
    echo -e "  ${D}VRAM headroom calculator for Arc A770 (16 GB)${N}"
    echo ""

    local total_vram_gb=16

    # Model VRAM estimates by quantisation (GB for 9B param model)
    declare -A q_size_9b=( [Q2_K]=3.5 [Q3_K_M]=4.5 [Q4_K_M]=5.5 [Q5_K_M]=6.5
                           [Q6_K]=7.5 [Q8_0]=9.5 [F16]=18.0 )

    echo -e "  ${W}Arc A770 VRAM budget (approximate for 9B models):${N}"
    echo ""
    printf "  ${D}%-10s  %-8s  %-20s  %-10s  %s${N}\n" "Quant" "Size" "KV@4096ctx" "KV@8192ctx" "Headroom@4096"

    for q in Q4_K_M Q5_K_M Q6_K Q8_0; do
        local model_gb="${q_size_9b[$q]:-?}"
        # KV cache ≈ 2 * n_layers * n_heads * head_dim * ctx * dtype_bytes / 1e9
        # For 9B: 32 layers, 32 heads, 128 head_dim, bf16 (2 bytes)
        # = 2 * 32 * 32 * 128 * ctx * 2 / 1e9
        local kv_4096_gb; kv_4096_gb=$(python3 -c "print(f'{2*32*32*128*4096*2/1e9:.2f}')" 2>/dev/null || echo "~1.1")
        local kv_8192_gb; kv_8192_gb=$(python3 -c "print(f'{2*32*32*128*8192*2/1e9:.2f}')" 2>/dev/null || echo "~2.1")
        local headroom; headroom=$(python3 -c "
m=${model_gb}; k=float('$kv_4096_gb'); t=${total_vram_gb}
rem=t-m-k-1.5  # 1.5GB for SYCL scratch/runtime
print(f'{rem:.1f}GB' if rem>0 else 'OOM RISK')
" 2>/dev/null || echo "?")
        printf "  %-10s  %-8s  %-20s  %-10s  %s\n" "$q" "${model_gb}GB" "${kv_4096_gb}GB" "${kv_8192_gb}GB" "$headroom"
    done

    echo ""
    print_row "$INFO" "Crash model detected" "Qwen3.5-9B.Q8_0 + 8192ctx + 99 layers = ~13GB → OOM on scratch alloc"
    print_row "$INFO" "Recommended fix"      "Use ctx-size=4096 + n-gpu-layers=85, or switch to Q4_K_M"
    echo ""

    # ── Scan server log for known SYCL error signatures ──
    local log_file="${HOME}/ai_stack/server.log"
    local arc_log="${HOME}/ai_stack/arc_fix.log"

    for logf in "$log_file" "$arc_log"; do
        [[ -f "$logf" ]] || continue
        local sycl_errs; sycl_errs=$(grep -c "could not create a memory object\|SYCL error\|CHECK_TRY_ERROR\|ggml_sycl" \
            "$logf" 2>/dev/null || echo 0)
        if (( sycl_errs > 0 )); then
            print_row "$WARN" "SYCL errors in ${logf##*/}" "${Y}${sycl_errs} occurrences found${N}"
            log_result WARN "SYCL OOM errors found in ${logf##*/}: $sycl_errs lines"
            echo ""
            echo -e "  ${D}Last SYCL error:${N}"
            grep "could not create a memory object\|SYCL error" "$logf" 2>/dev/null | tail -2 | \
                while read -r line; do echo -e "  ${D}  $line${N}"; done
        else
            print_row "$PASS" "No SYCL errors in ${logf##*/}" ""
            log_result PASS "No SYCL OOM errors in log"
        fi
    done

    # ── Offer to write an Arc-optimised ini template ──
    echo ""
    echo -e "  ${W}[a]${N}  Write Arc A770-optimised .ini template for current model dir"
    echo -e "  ${W}[s]${N}  Skip"
    echo ""
    read -rp "  Choice [a/s]: " ch
    [[ "${ch,,}" == a ]] && _write_arc_ini_template
}

# Write ~/ai_stack/llama_configs/arc-a770-optimised.ini
_write_arc_ini_template() {
    local config_dir="${HOME}/ai_stack/llama_configs"
    mkdir -p "$config_dir"
    local out="${config_dir}/arc-a770-optimised.ini"

    cat > "$out" << 'ARC_INI'
# Arc A770 (16 GB VRAM) — optimised settings
# Generated by arc-fix.sh  (avoids SYCL "could not create a memory object" OOM)
#
# Key tunings:
#   n-gpu-layers = 85  — reserves ~2 GB for SYCL scratch buffers
#   ctx-size     = 4096 — safe KV cache footprint (~1.1 GB for 9B models)
#   batch-size   = 256  — lower peak allocation during prompt processing
#
# Env vars that MUST be set before launch (arc-fix.sh writes these to
...
model           = /home/first/ai_stack/models/your-model.gguf
n-gpu-layers    = 85
ctx-size        = 4096
batch-size      = 256
threads         = 4
...
# Server settings (uncomment to use as server config)
# host = 127.0.0.1
# port = 8080
# parallel = 1
ARC_INI
    ...
    OK "Written: $out"
    record_fix "Created Arc A770-optimised INI template at $out"
    log_result PASS "Arc A770 optimised .ini template written"
    sleep 1
}
...
show_menu() {
    ...
    case "${choice,,}" in
        ...
        s|S)  check_sycl_memory;    print_report ;;
