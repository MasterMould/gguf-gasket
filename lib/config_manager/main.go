...
// ─── Shared state (~/ai_stack/selected.env) ──────────────────────────────────
...
func loadGasketState() GasketState {
	...
	for scanner.Scan() {
		...
		switch key {
		case "STATE_MODEL":  st.Model  = val
		case "STATE_CONFIG": st.Config = val
		case "STATE_BINARY": st.Binary = val
		case "STATE_PORT":   st.Port   = val
		}
	}
	return st
}
...
func saveGasketState(st GasketState) error {
	...
	content := fmt.Sprintf(
		"# gguf-gasket state — written by config manager\n"+
			"STATE_MODEL=%s\nSTATE_CONFIG=%s\nSTATE_BINARY=%s\nSTATE_PORT=%s\n",
...
// ─── Server lifecycle (llama-server process management) ───────────────────────
...
func serverStatus() ServerStatus {
	...
	st.Running = true
	st.PID = pid
	// Try to read mtime of PID file as proxy for uptime
	if info, err := os.Stat(serverPIDFile()); err == nil {
		d := time.Since(info.ModTime()).Round(time.Second)
		st.Uptime = d.String()
	}
	gs := loadGasketState()
	st.Port   = gs.Port
...
type ServerStartReq struct {
	...
	Model  string `json:"model"`  // override model path (optional)
	Binary string `json:"binary"` // override llama-server path (optional)
	Port   string `json:"port"`
...
// ─── Arc GPU status (runs arc_check.sh functions via shell) ──────────────────
...
func queryArcStatus() ArcStatus {
	...
	arcIDs := []string{"56a0","56a1","56a5","56a6","5690","5691","5692","56b0","56b1","56c0","56c1"}
	...
	for _, drv := range []string{"xe", "i915"} {
		if strings.Contains(string(out), drv+" ") {
			st.DriverOK = true
			st.DriverName = drv
			break
		}
	...
	if len(matches) > 0 {
		...
		if err == nil { f.Close(); st.RenderOK = true }
	}
	...
	// Level Zero package
	out, _ = exec.Command("dpkg", "-l", "libze-intel-gpu1").Output()
	st.LevelZeroOK = strings.Contains(string(out), "ii")

	// Summary
	issues := 0
	if !st.DriverOK { issues++ }
	if !st.RenderOK { issues++ }
	if !st.GroupsOK { issues++ }
	if issues == 0 {
		st.Summary = fmt.Sprintf("OK — driver=%s render=/dev/dri/renderD*", st.DriverName)
	} else {
...
// GET /api/state — read shared gasket state
func (h *Handler) getState(w http.ResponseWriter, r *http.Request) {
	wire(w, 200, loadGasketState())
}

// PUT /api/state — write shared gasket state
func (h *Handler) putState(w http.ResponseWriter, r *http.Request) {
	var st GasketState
	if json.NewDecoder(r.Body).Decode(&st) != nil {
		errJSON(w, 400, "bad JSON"); return
	}
...
// POST /api/server/start — parse .ini, start llama-server subprocess
func (h *Handler) serverStart(w http.ResponseWriter, r *http.Request) {
	...
	if serverStatus().Running {
		errJSON(w, 409, "server already running"); return
	}
	...
	if bin == "" {
		errJSON(w, 422, "no llama-server binary found — set in Paths or pass binary field"); return
	}
	...
	if model == "" {
		errJSON(w, 422, "no model selected — pick one in the Models tab first"); return
	}
	...
	hasPort := false
	for _, a := range args { if a == "--port" { hasPort = true; break } }
	if !hasPort { args = append(args, "--port", port) }

	if req.Extra != "" {
		for _, part := range strings.Fields(req.Extra) { args = append(args, part) }
	}
	...
	if err != nil {
		errJSON(w, 500, "cannot open log: "+err.Error()); return
	}
	...
	if err := cmd.Start(); err != nil {
		f.Close()
		errJSON(w, 500, "start failed: "+err.Error()); return
	}
	f.Close()

	// Write PID and update shared state
	os.WriteFile(serverPIDFile(), []byte(strconv.Itoa(cmd.Process.Pid)), 0644)
	gs := loadGasketState()
	gs.Model  = model
	gs.Config = req.Config
	gs.Port   = port
	saveGasketState(gs)

	wire(w, 200, map[string]any{
		"pid":  cmd.Process.Pid,
		"port": port,
		"log":  logFile,
	})
...
func (h *Handler) serverStop(w http.ResponseWriter, r *http.Request) {
	st := serverStatus()
	if !st.Running {
		errJSON(w, 409, "server not running"); return
	}
...
// GET /api/server/log?n=80 — last n lines of server.log
func (h *Handler) serverLog(w http.ResponseWriter, r *http.Request) {
	n := 80
	if v := r.URL.Query().Get("n"); v != "" {
		if i, err := strconv.Atoi(v); err == nil && i > 0 { n = i }
	}
	data, err := os.ReadFile(serverLogFile())
	if err != nil {
		wire(w, 200, map[string]string{"log": ""}); return
	}
	lines := strings.Split(string(data), "\n")
	if len(lines) > n { lines = lines[len(lines)-n:] }
	wire(w, 200, map[string]string{"log": strings.Join(lines, "\n")})
}

// GET /api/arc/status — quick Arc GPU check
...
func templateFile(name, tmpl string) IniFile {
	...
	case "arc-a770":
		return IniFile{Name: name, Sections: []IniSection{{Name: "", Keys: []IniKey{
			...
			{Key: "n-gpu-layers", Value: "85", Comment: "85 of 99 layers — reserves ~2GB for SYCL scratch (prevents OOM)"},
			{Key: "ctx-size", Value: "4096", Comment: "safe KV cache footprint on Arc A770 16GB (~1.1GB for 9B models)"},
			{Key: "batch-size", Value: "256", Comment: "lower peak allocation during prompt processing"},
			{Key: "threads", Value: "4", Comment: "CPU threads for non-offloaded layers"},
			{Key: "flash-attn", Value: "true", Comment: "Flash Attention — faster on Arc SYCL"},
			{Key: "cont-batching", Value: "true", Comment: "continuous batching for throughput"},
			{Key: "temperature", Value: "0.7", Comment: "sampling temperature"},
			{Key: "top-p", Value: "0.9", Comment: "nucleus sampling"},
			{Key: "repeat-penalty", Value: "1.1", Comment: "repeat penalty"},
			{Key: "n-predict", Value: "-1", Comment: "max tokens (-1 = unlimited)"},
			{Key: "seed", Value: "-1", Comment: "RNG seed (-1 = random)"},
		}}}}
	default:
		return IniFile{Name: name, Sections: []IniSection{{Name: "", Keys: []IniKey{
...
.dot-run  { background: var(--accent); box-shadow: 0 0 6px var(--accent); }
.dot-stop { background: var(--text3); }
.server-info { color: var(--text2); }
.server-info b { color: var(--text); }
.btn-srv-launch {
  background: var(--accent); color: var(--bg); border: none;
  border-radius: var(--radius); font-family: var(--mono); font-size: 11px;
  font-weight: 700; padding: 3px 12px; cursor: pointer; transition: opacity .15s;
  white-space: nowrap;
}
.btn-srv-launch:hover { opacity: .85; }
.btn-srv-stop {
  background: transparent; color: var(--danger); border: 1px solid var(--danger);
  border-radius: var(--radius); font-family: var(--mono); font-size: 11px;
  padding: 3px 10px; cursor: pointer; transition: all .15s; white-space: nowrap;
}
.btn-srv-stop:hover { background: var(--danger); color: #fff; }
.arc-badge {
...
.arc-badge.ok   { color: var(--accent2); border-color: var(--accent2); }
.arc-badge.warn { color: var(--accent3); border-color: var(--accent3); }
.arc-badge.err  { color: var(--danger);  border-color: var(--danger); }
/* log viewer */
.log-viewer {
  font-family: var(--mono); font-size: 11px; color: var(--text2);
  background: var(--bg); border: 1px solid var(--border); border-radius: var(--radius);
  padding: 10px 12px; height: 180px; overflow-y: auto; white-space: pre;
...
<!-- ── Server & Arc status bar ─────────────────────────────────────────────── -->
<div class="server-bar" id="server-bar">
  <div class="server-dot dot-stop" id="srv-dot"></div>
  <span class="server-info" id="srv-info">llama-server: checking…</span>
  <button class="btn-srv-launch" id="btn-srv-launch" onclick="launchServer()" style="display:none">
    ▶ Launch server
  </button>
  <button class="btn-srv-stop" id="btn-srv-stop" onclick="stopServer()" style="display:none">
    ■ Stop
  </button>
  <div class="arc-badge" id="arc-badge" title="Intel Arc GPU status">🔮 Arc: …</div>
</div>
...
<div class="modal-overlay" id="new-modal" style="display:none">
  <div class="modal" onclick="event.stopPropagation()">
    ...
    <select id="new-template">
      ...
      <option value="arc-a770">Arc A770 optimised</option>
    </select>
...
<script>
// ── Server & Arc status polling ──────────────────────────────────────────────
let serverRunning = false;
let arcOk = false;

async function pollServerStatus() {
  try {
    const st = await api('GET', '/server/status');
    serverRunning = st.running;
    const dot  = document.getElementById('srv-dot');
    const info = document.getElementById('srv-info');
    const btnL = document.getElementById('btn-srv-launch');
    const btnS = document.getElementById('btn-srv-stop');

    if (st.running) {
      dot.className = 'server-dot dot-run';
      info.innerHTML = 'llama-server: <b>RUNNING</b>  PID ' + st.pid + '  port ' + st.port + '  model ' + (st.model_name||'?') + '  uptime ' + st.uptime;
      btnL.style.display = 'none';
      btnS.style.display = '';
    } else {
      dot.className = 'server-dot dot-stop';
      info.innerHTML = 'llama-server: <b>stopped</b>';
      btnL.style.display = current ? '' : 'none';
      btnS.style.display = 'none';
    }
  } catch(e) {
    document.getElementById('srv-info').textContent = 'llama-server: unavailable';
  }
}

async function pollArcStatus() {
  try {
    const arc = await api('GET', '/arc/status');
    arcOk = arc.detected && arc.driver_ok && arc.render_ok;
    const badge = document.getElementById('arc-badge');
    if (!arc.detected) {
      badge.className = 'arc-badge err'; badge.textContent = '🔮 Arc: not found';
    } else if (arcOk) {
      badge.className = 'arc-badge ok';
      badge.textContent = '🔮 Arc: OK  ' + arc.driver_name;
    } else {
      badge.className = 'arc-badge warn';
      badge.textContent = '🔮 Arc: ' + arc.summary;
    }
    badge.title = arc.summary;
  } catch(e) { /* lspci may not be available */ }
}

async function loadGasketState() {
  try { return await api('GET', '/state'); } catch(e) { return {}; }
}
async function saveGasketState(s) {
  try { return await api('PUT', '/state', s); } catch(e) { return {}; }
}

// Launch llama-server with currently open config
async function launchServer() {
  if (!current) { toast('Open a config file first', true); return; }
  if (activeTab === 'raw') parseRaw();
  // Auto-save first
  try { await api('PUT', '/files/'+current.Name, current); } catch(e) {}

  // Get current state for model
  const st = await loadGasketState();
  if (!st.model) { toast('No model selected — pick one in the Models tab', true); return; }
  ...
  try {
    ...
    toast('Server started — PID ' + res.pid + '  port ' + res.port);
    pollServerStatus();
...
setInterval(pollArcStatus, 30000);
pollServerStatus();
pollArcStatus();
...
async function openFile(name) {
  ...
  // Show launch button if server is stopped
  const btnL = document.getElementById('btn-srv-launch');
  if (btnL && !serverRunning) btnL.style.display = '';
  // Sync config selection to shared state
  saveGasketState({ config: name }).catch(()=>{});
}
...
function useModel(path) {
  ...
  // Persist to shared gasket state so manager.sh / menu_server.sh see it too
  saveGasketState({ model: path }).catch(()=>{});
  // Show launch button now that a model is selected
  const btnL = document.getElementById('btn-srv-launch');
  if (btnL && !serverRunning) btnL.style.display = '';
}
