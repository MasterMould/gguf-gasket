...
#
#  SearXNG aggregates results from 70+ search engines with no
#  tracking. When paired with llama-server it gives the LLM
#  live web search capability via a JSON API.
#
#  This module:
#    • Installs SearXNG via Docker (recommended) or bare Python
#    • Generates a secure random secret key
#    • Writes a minimal settings.yml (JSON API enabled, limiter off)
#    • Manages container start / stop / restart
#    • Verifies the JSON search endpoint is reachable
#    • Shows the endpoint URL for use in LLM tool-calling configs
# ================================================================
...
_searxng_generate_secret() {
    # 32-byte hex secret — required by SearXNG
    openssl rand -hex 32 2>/dev/null || python3 -c \
...
_searxng_install_docker() {
    ...
    docker run -d \
        ...
        -p "127.0.0.1:${SEARXNG_PORT}:${SEARXNG_PORT}" \
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] searxng installed, port=$SEARXNG_PORT" >> "$LOG_FILE"
    sleep 4
}
